...
evaluator → **keep** (commit) if it improves the metric, else **revert**
(`git reset --hard HEAD~1`). The codebase only ratchets forward; the git log is the
audit trail and your memory across runs.
...
3. **Edit** the sandbox file(s). **Commit.**
4. **Run** the evaluator via the execution adapter under the budget. Redirect logs
   to a file; **grep the scalar** — never read giant logs wholesale.
5. **If it crashed/timed out:** attempt a fix a few times; else revert and log
   `status=crash`. Do not stall waiting for a human.
6. **If the scalar improved and no veto fired:** keep the commit (new baseline).
   **Else:** `git reset --hard HEAD~1`.
7. **Append a row** to the ledger: `experiment, commit, metric, delta, status,
   description`.
